...
function compactCandidates(items) {
  return (items || []).map(x => `${x.fileId}:${x.version || ''}:${x.name || ''}`).join(' | ');
}
...
function main() {
  ...
  if (!planFile || !closureFile) {
    console.error('用法: node build-review-queue.js <plan.json> <closure.json> --out review.json [--tsv review.tsv]');
    process.exit(2);
  }
  ...
  for (const item of plan.items || []) {
    ...
    if (!mainHold && !closureHold) continue;

    const next = [];
    if (['HOLD_REVIEW', 'HOLD_AMBIGUOUS', 'HOLD_LOW_CONFIDENCE', 'HOLD_SAME_VERSION_REPLACEMENT'].includes(item.action)) {
      next.push('核对 Files 页面与当前安装来源，明确正确 Main File/变体；禁止按上传时间直接选');
    }
    ...
    if (c?.conflicts?.length) next.push('扫描证据与 registry 冲突，必须逐个解释候选，不得直接写 NONE');
    if (c?.invalidRules?.length) next.push('修复 registry v2 记录，并确保 REQUIRED 附属 fileId 已通过 registry audit');

    queue.push({
      priority: item.action === 'DOWNLOAD' ? 'P1-CLOSURE' : 'P0-MAIN',
      modId: item.modId,
      localName: item.name,
      ...
      targetName: item.latestName || '',
      mainAction: item.action,
      confidence: item.confidence,
      margin: item.margin,
      topCandidates: item.candidates || [],
      patchCandidates: item.aux?.patches || [],
      translationCandidates: item.aux?.translations || [],
      closure: c || null,
      nextActions: next,
    });
  }
  ...
  if (tsvFile) {
    const header = [
      'priority', 'modId', 'localName', 'localFileId', 'localVersion',
      'targetFileId', 'targetVersion', 'targetName', 'mainAction', 'confidence',
      'patchCandidates', 'translationCandidates', 'nextActions',
    ];
    const lines = [header.join('\t')];
    for (const q of queue) lines.push([
      q.priority, q.modId, q.localName, q.localFileId, q.localVersion,
      q.targetFileId, q.targetVersion, q.targetName, q.mainAction, q.confidence,
      compactCandidates(q.patchCandidates), compactCandidates(q.translationCandidates), q.nextActions.join(' / '),
