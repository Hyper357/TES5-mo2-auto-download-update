...
assert.strictEqual(rel({
  ...
  kind: 'CONFIG',
  requiredHint: true,
  name: 'general syntax of the document required for these settings',
}).blocking, false);
assert.strictEqual(rel({
  source: 'DESCRIPTION_TEXT',
  kind: 'OPTIONAL_COMPONENT',
  requiredHint: true,
  name: 'mark pieces as required, optional or locked',
}).blocking, false);
assert.strictEqual(rel({
  source: 'DESCRIPTION_TEXT',
  kind: 'PATCH',
  requiredHint: true,
  name: "It's not required to use both Icy Fixes and Meshes together.",
}).blocking, false);
...
assert.strictEqual(rel({
  ...
  installedContextMatch: true,
}).blocking, false);
assert.strictEqual(rel({
  source: 'DESCRIPTION_LINK',
  kind: 'PHYSICS',
  name: 'FSMP - Faster HDT-SMP',
  installedContextMatch: true,
}).blocking, false);
...
assert.strictEqual(rel({
  ...
  family: 'CUSTOM:OTHER_ARMOR',
  name: 'Children of the First - Skypatcher Patch - GDPR',
}).blocking, false);
assert.strictEqual(rel({
  source: 'SAME_PAGE_FILE',
  kind: 'MESH',
  category: 'MISCELLANEOUS',
  family: 'CUSTOM:OTHER_MODEL',
  name: 'Other armor meshes - GDPR',
}).blocking, false);

// A same-page compatibility patch becomes blocking only with positive active counterpart evidence.
assert.strictEqual(rel({
  source: 'SAME_PAGE_FILE',
  kind: 'PATCH',
  category: 'OPTIONAL FILES',
  family: 'LUX_ORBIS',
  name: 'Lux Orbis compatibility patch',
  optionalHint: false,
  environmentDecision: { resolved: false, status: 'UNRESOLVED', reason: 'COMPAT_COUNTERPART_ENABLED', evidence: ['Lux Orbis.esp'] },
}).blocking, false); // explicit Optional category wins
assert.strictEqual(rel({
  source: 'SAME_PAGE_FILE',
  kind: 'PATCH',
  category: '',
...
const falseOnly = assessComponentDiscovery({
  ...
  rules: [],
  coverage: { description: { required: true, complete: true, status: 'COMPLETE' } },
  mainName: 'QuickLootRE',
  mainVersion: '2.3.6',
});
...
const translation = assessComponentDiscovery({
  candidates: [
    { kind: 'TRANSLATION', source: 'DESCRIPTION_LINK', name: 'Mandarin Translation', family: 'ZH_CN', decision: { resolved: false }, environmentDecision: { reason: 'NO_SAFE_INFERENCE' } },
  ],
  rules: [],
  coverage: { description: { required: true, complete: true, status: 'COMPLETE' } },
  mainName: 'Icy Mesh Remaster',
  mainVersion: '2.2.1',
});
