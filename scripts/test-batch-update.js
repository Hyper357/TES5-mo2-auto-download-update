...
try {
  ...
  writeMeta(mods, 'Mod C', { modId: 3, version: '1.0', fileId: 30 });

  const rows = [
    { modId: '1', fileId: '11', ver: '1.1', name: 'Mod A', note: '', action: 'DOWNLOAD' },
    { modId: '101', fileId: '111', ver: '1.0', name: 'Required Resource A', note: 'tx=1:11; closure:RESOURCE:CUSTOM:FRAMEWORK', action: 'DOWNLOAD' },
    { modId: '2', fileId: '21', ver: '2.0', name: 'Mod B', note: '', action: 'DOWNLOAD' },
    { modId: '3', fileId: '31', ver: '1.1', name: 'Mod C', note: '', action: 'DOWNLOAD' },
    { modId: '303', fileId: '331', ver: '1.0', name: 'Windhelm Is Snowy - Capital Windhelm', note: 'tx=3:31; closure:PATCH:CUSTOM:CAPITAL_WINDHELM', action: 'DOWNLOAD' },
  ];
  const groups = groupDownloadRows(rows);
  assert.strictEqual(groups.size, 3);
  assert.strictEqual(groups.get('1:11').length, 2);

  const emptyProfile = { profile: { usableForApplicability: true }, mods: [], missingModlistEntries: [], plugins: [] };
  const first = chooseTransactions({ groups, count: 20, modsDir: mods, downloadsDir: downloads, environmentGraph: emptyProfile });
  ...
  assert.ok(first.excluded.some(x => x.tx === '3:31' && x.reason === 'AUX_APPLICABILITY_UNPROVEN'));
  ...
  const second = chooseTransactions({ groups, count: 20, modsDir: mods, downloadsDir: downloads, environmentGraph: emptyProfile });
