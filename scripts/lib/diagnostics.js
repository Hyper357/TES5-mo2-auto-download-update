...
const CODES = Object.freeze({
  ...
  MO2_UI_UNAVAILABLE: { retry: false, layer: 'MO2_UI', action: 'UIAutomation 不可用时退回磁盘/ledger/verify 证据；不要通过模糊点击补偿。' },
  MO2_DIALOG_AMBIGUOUS: { retry: false, layer: 'MO2_UI', action: '检测到疑似重复下载弹窗，但无法与当前目标可靠匹配。不要自动点击，人工/Pi 核对弹窗内容。' },
  MO2_REDOWNLOAD_BLOCKED: { retry: false, layer: 'MO2_UI', action: '已安全选择“否”取消重复下载。检查现有队列/归档是否就是目标 fileId；禁止立即再次提交。' },
...
function classifyFailure(input) {
  ...
  const rules = [
    ...
    [/MO2_DIALOG_AMBIGUOUS|dialog.*ambiguous|ambiguous.*dialog/i, 'MO2_DIALOG_AMBIGUOUS'],
    [/MO2_REDOWNLOAD_BLOCKED|redownload.*blocked|重新下载.*取消/i, 'MO2_REDOWNLOAD_BLOCKED'],
    [/MO2_UI_UNAVAILABLE|uiautomation.*(?:unavailable|failed)|ui automation.*(?:unavailable|failed)/i, 'MO2_UI_UNAVAILABLE'],
