...
const CODES = Object.freeze({
  ...
  NEXUS_LOGIN_REQUIRED: { retry: false, layer: 'BROWSER', action: '在 CDP 浏览器中重新登录 Nexus。' },
  CDP_UNAVAILABLE: { retry: true, layer: 'BROWSER', action: '确认调试浏览器在 127.0.0.1:9222 运行。' },
  ...
  NXM_HANDLER_FAILED: { retry: true, layer: 'MO2', action: '检查 nxmhandler 与当前 MO2 实例绑定。' },
  ...
  MO2_QUEUE_NOT_FOUND: { retry: true, layer: 'MO2', action: '刷新/唤醒 MO2 Downloads 后重试一次。' },
  MO2_DOWNLOAD_STALLED: { retry: true, layer: 'DOWNLOAD', action: '检查 CDN/队列；只重试当前精确 fileId。' },
  ARCHIVE_NOT_LANDED: { retry: true, layer: 'DOWNLOAD', action: '确认下载队列和 CDN 状态后重试当前项。' },
  META_MISSING: { retry: true, layer: 'VERIFY', action: '等待 MO2 完成 .meta 落盘；超时后重试当前项。' },
  ...
  ARCHIVE_CORRUPT: { retry: true, layer: 'VERIFY', action: '删除/隔离损坏下载后重新下载精确 fileId。' },
  ...
  VERIFY_TIMEOUT: { retry: true, layer: 'VERIFY', action: '检查下载是否仍增长；必要时仅重试当前项。' },
...
function classifyFailure(input) {
  ...
  const rules = [
    ...
    [/ECONNREFUSED.*9222|cdp.*(?:failed|unavailable|connect)|127\.0\.0\.1:9222.*refused/i, 'CDP_UNAVAILABLE'],
