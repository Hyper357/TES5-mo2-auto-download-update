...
const {
  inferFamily, mergeCandidates, installedContext, assessDiscovery, normalizeFamilyKey,
...
const coverage = {
  samePageFiles: { required: true, complete: true, status: 'COMPLETE' },
  requirementsReverse: { required: true, complete: true, status: 'COMPLETE' },
  description: { required: true, complete: true, status: 'COMPLETE' },
};
...
assert.strictEqual(one.unresolved.length, 0);

const unresolved = assessDiscovery({ candidates: [ussep], patchRules: [], coverage });
assert.strictEqual(unresolved.complete, false);
assert.strictEqual(unresolved.unresolved.length, 1);

const badCoverage = assessDiscovery({
  candidates: [], patchRules: [],
  coverage: { requirementsReverse: { required: true, complete: false, status: 'SECTION_NOT_PROVEN' } },
});
assert.strictEqual(badCoverage.complete, false);
assert.strictEqual(badCoverage.coverageProblems.length, 1);
