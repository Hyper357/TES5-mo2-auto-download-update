...
function httpJson(url, timeout = 5000) {
  return new Promise(resolve => {
    ...
    const req = client.get(url, { timeout, headers: { 'User-Agent': 'TES5-mo2-auto-download-update/3.2-diagnostics' } }, res => {
      let body = '';
      res.on('data', c => body += c);
      res.on('end', () => {
        try { resolve({ ok: res.statusCode >= 200 && res.statusCode < 300, status: res.statusCode, json: JSON.parse(body) }); }
        catch { resolve({ ok: false, status: res.statusCode, error: 'INVALID_JSON' }); }
      });
    });
...
function nexusValidate(apiKey, timeout = 8000) {
  return new Promise(resolve => {
    ...
    const req = https.get({
      ...
      headers: { apikey: apiKey, Accept: 'application/json', 'User-Agent': 'TES5-mo2-auto-download-update/3.2-diagnostics' },
    }, res => {
      let body = '';
      res.on('data', c => body += c);
      res.on('end', () => {
...
async function main() {
  ...
  const cdp = await httpJson('http://127.0.0.1:9222/json/version');
  ...
  const checks = [
    ...
    status('CDP_9222', cdp.ok, cdp.ok ? { browser: cdp.json?.Browser, protocol: cdp.json?.['Protocol-Version'] } : cdp),
  ...
  const report = sanitize({
    ...
    paths: { modsDir, downloadsDir, apiKeyFile, nxmHandler, sevenzip },
    checks,
  ...
  console.log(JSON.stringify({ health, reportFile, summary: report.summary, checks }, null, 2));
